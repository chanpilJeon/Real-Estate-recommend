...
export interface ScoreWeights { price: number; liquidity: number; location: number; quality: number }
...
export interface ScoreBreakdown extends ScoreWeights { total: number; reasons: string[]; missingData: string[] }
...
export class ScoringPolicy {
  constructor(private readonly weights: ScoreWeights) {
    ...
    if (values.some(v => !Number.isFinite(v) || v < 0) || Math.abs(values.reduce((a, b) => a + b, 0) - 1) > 0.000001) throw new Error('점수 가중치 합은 1이어야 합니다.');
  }
  static preset(name: PresetName) { return new ScoringPolicy(WEIGHTS[name]); }
  score(input: ScoreInput): ScoreBreakdown {
    ...
    if (input.budgetMaxManwon === null || input.budgetMaxManwon <= 0) missingData.push('예산 상한 미설정');
    ...
    else reasons.push(`최근 1년 거래량 / 세대수 ${(input.liquidity * 100).toFixed(1)}%`);
    const distanceScore = (meters: number | null, max: number, label: string) => {
      if (meters === null) { missingData.push(label); return 0.5; }
      reasons.push(`${label} 직선 ${meters.toLocaleString()}m`);
      return clamp(1 - meters / max);
    };
    const location = distanceScore(input.nearestSubwayM, 1500, '지하철역') * 0.7 + distanceScore(input.nearestSchoolM, 1000, '초등학교') * 0.3;
    reasons.push(...input.qualityReasons);
    const quality = clamp(input.quality);
    const total = 100 * (price * this.weights.price + liquidity * this.weights.liquidity + location * this.weights.location + quality * this.weights.quality);
    return { total: round(total), price: round(price * 100), liquidity: round(liquidity * 100), location: round(location * 100), quality: round(quality * 100), reasons, missingData };
  }
}
