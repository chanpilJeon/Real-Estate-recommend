...
매일 06:00 자동 수집은 맥이 켜져 있을 때만 돕니다. 그래서 **켤 때 밀린 수집을 자동으로
따라잡도록** 해뒀습니다 — 마지막 수집이 하루 가까이 지났으면 서버가 뜨고 10초 뒤에
조용히 한 번 돕니다. 껐다 켤 때마다 돌지는 않습니다.
...
pnpm verify:molit   # 우리 데이터가 국토부 원본과 맞는지 대조 (--regions 11680)
...
개발자용: 최초 1회 `pnpm exec playwright install chromium`, 이후 `pnpm test:e2e`. PC·모바일 검색 → 목록 → 상세와 신고 초안을 확인합니다. 이 테스트의 API 응답은 고정 샘플이므로 실제 공공데이터 정확성 검증과는 별개입니다. 화면 우하단 **문제 신고**로 검색조건을 담은 초안을 만들 수 있습니다.

### 추천 기능 사용하기 (Step 7)

1. Docker Desktop을 실행하고 Finder에서 `scripts/start.command`를 더블클릭하세요. 검은 터미널 창은 열어두세요.
2. [서비스 화면](http://localhost:3000)에서 **강남구**를 검색하고 후보를 선택하세요.
3. 예산 상한(억 원)·면적을 입력한 뒤 **추천 기준**을 골라보세요: 가성비형 / 입지우선형 / 신축선호형.
4. 카드에서 점수와 추천 이유를 확인하고, 단지를 누른 후 전용면적을 골라 가격 추이·전세가율을 확인하세요.
5. 이미 서버가 켜져 있으면 웹을 새로고침하면 됩니다. 추천 기준이 없다면 터미널에서 `Ctrl+C`로 종료하고 `pnpm dev`로 다시 켜세요.

추천은 현재 매물이나 투자 수익률 예측이 아닌 **수집된 실거래와 단지 정보의 조건 비교**입니다. 예산 상한을 생략하면 가격 점수는 중립으로 둡니다. 입지 미수집 지역은 카드의 미확인 항목에 표시됩니다. 신규 추천 계산식과 검증 범위는 [추천 기준 설명](docs/RECOMMENDATION.md)에 적었습니다.

### 배포 준비 상태

API Docker 이미지, Caddy 구성, 배포 스크립트, GitHub Actions 검증을 추가했습니다. 실제 클라우드 서버·도메인·스테이징 주소는 아직 연결하지 않았습니다. 자세한 절차는 [배포 가이드](docs/DEPLOYMENT.md)를 참고하세요. 로컬 이용에는 필요하지 않습니다.

2026-09-08 입지 수집: 강남구 718개, 하남시 138개 단지의 좌표를 기준으로 역·초등학교 거리를 계산했습니다. 다른 지역의 입지 미확인 항목은 중립 점수로 표시됩니다.
