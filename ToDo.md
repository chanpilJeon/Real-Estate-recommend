...
> **M3 1차 검증 완료 (2026-09-09)** — `pnpm verify:molit` 로 자동화. docs/TEST-CHECKLIST.md 참조.
> 계약일·전용면적·층·거래금액은 **전 건 100% 일치** (강남구 230 · 영통구 384 · 분당구 591건).
> 대조하다 **취소된 거래가 시세에 섞이는 버그**를 찾아 고쳤다 — 국토부가 같은 계약을
> 원거래·해제거래 두 줄로 주는데, 중복 판정에서 먼저 온 쪽만 남아 해제가 유실되고 있었다.
> **20,500건(전체의 3.3%)이 취소됐는데도 시세에 잡히고 있었다.** 수정 후 재대조 결과 합격.
...
> **M5 1차 검증 (2026-09-10, 광교·예산 10억)** — docs/TEST-CHECKLIST.md 참조.
> 미리 적은 대장 5곳이 상위 20위에 **0곳**. 원인은 엔진이 아니라 예산이었다 —
> 광교 대장은 전부 10.7억~13.8억으로 예산 밖이고, 빼는 것이 옳다.
> 진짜 문제는 **63곳 중 19곳만 조용히 보여준 것.** 이유를 한 문장으로 말하도록 고쳤다.
> 추천 근거 문장(거래량·주차·역거리)은 원자료와 전부 일치했다.
>
> **M5 2차 (같은 광교·예산 15억)** — 예산 안에서 살 수 있는 3곳 중 **2곳이 상위 20위**.
> 이 회차에서 **큰 평형이 단지를 통째로 지우던 결함**을 찾아 고쳤다 (서울·경기 115곳 해당).

> **M5 1차 검증 (2026-09-08, 영통동 예산 7억)** — 처음엔 납득되지 않았다.
> `myown`(14㎡ 원룸, 9,700만원)이 1위였다. 가격 점수가 `1 - 가격/예산` 이라
> **쌀수록 높은 점수**였기 때문이다. 집을 사려는 사람은 가장 싼 것이 아니라
> 예산 안에서 가장 좋은 집을 원한다.
>
> 고친 뒤: 예산의 71~85%를 쓰는 4.9~6억대 아파트가 상위에 온다.
> - 가격 점수를 **예산 활용도 곡선**으로 (70% 지점 최고, 상한에 가까울수록 완만히 감점)
> - 추천에 한해 **전용 40㎡ 미만 기본 제외** (검색으로는 여전히 볼 수 있다)

---

### Step 8 — 배포 + 스테이징 (1주) ★ 비기술자 테스트가 쉬워지는 분기점

> **보류 (2026-09-10).** 배포하지 않고 **로컬에서 혼자 쓰기로** 결정.
> 필요해지면 이 절을 그대로 따르면 된다. 대신 로컬 운영에 필요한 것을 아래에 정리했다.
...
> **M7 1차 검증 (2026-09-08)** — 실제 데이터로 띄워 보고 세 가지를 고쳤다.
> 1. 대시보드가 **없는 일감을 만들어냈다.** 실제 미매칭 거래는 0건인데 과거 실패 기록
>    476건이 남아 "476건이 연결되지 않았습니다"로 표시됐다. 수집 후 자동으로 닫도록 수정.
> 2. **"국토부 API 17,083 / 10,000"** — 한도를 넘긴 숫자. API 4종을 한 통에 세고 있었다.
>    실거래(10,000)와 단지정보(5,000)로 분리.
> 3. 한도 초과를 빨간불로 띄웠다 → 자정에 저절로 풀리는 일이라 경고로 낮춤.

---

### Step 10 — Phase 4 사용자 계정 (1~2주) — **보류 (혼자 쓰므로 불필요)**
...
## 6-B. 로컬에서 혼자 쓰기 (배포 대신)

배포하지 않고 내 맥에서만 쓰는 경우에 필요한 것.

### 켜고 끄기

```bash
./scripts/start.command      # 더블클릭해도 된다
```

화면은 http://localhost:3000 · 관리자는 http://localhost:3000/admin

### 데이터는 알아서 최신이 된다

| 언제 | 무엇이 |
|---|---|
| 매일 06:00 | 최근 2개월 실거래 + 단지 정보 보강 (맥이 켜져 있을 때) |
| **켤 때마다** | 마지막 수집이 하루 가까이 지났으면 **밀린 것을 자동으로 따라잡는다** |

밤에 노트북을 닫으면 06:00 cron 이 아예 돌지 않는다. 그래서 **켤 때 따라잡는 장치**를
따로 뒀다 (`CatchUpCollectionJob`). 켤 때마다 도는 것은 아니고, 마지막 수집이
20시간을 넘겼을 때만 한 번 돈다 — 안 그러면 껐다 켤 때마다 공공 API 한도를 쓴다.

### 가끔 확인할 것

- `/admin` 을 열어 맨 위 한 문장만 본다 ("정상입니다" / "확인이 필요합니다")
- 한 달에 한 번쯤 `pnpm verify:molit --regions 41117` 로 국토부 원본과 대조

### 하지 않아도 되는 것

- **배포(Step 8)** — 인터넷에 올리지 않으므로 AWS·도메인·모니터링이 필요 없다
- **사용자 계정(Step 10)** — 혼자 쓰므로 로그인이 필요 없다.
  관리자 로그인은 이미 있고, 그것만으로 충분하다
